...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventMouseMotion:
		...
		if plot_rect.has_point(get_node("PlotArea").get_local_mouse_position()):
			...
			get_node("PlotArea/Coordinate").text = "Time " + str(snapped(point.x, 0.1)) + ", " + str(snapped(point.y, 1)) + " Speed"
...
func update_graph() -> void:
	...
	var margin_left: float = MARGIN_LEFT if get_node("Axis").y_label == "" else MARGIN_LEFT + 25
	var margin_bottom: float = MARGIN_BOTTOM if get_node("Axis").x_label == "" else MARGIN_BOTTOM + 25
	...
	var y_axis_range: int = y_max - y_min
	...
	var x_axis_range: int = x_max - x_min
	...
	for n in range(vert_grad_number):
		...
		grad.append(grad_px)
		var grad_text = str(float(y_max) - n * float(y_axis_range)/(vert_grad_number-1))
	...
	for n in range(hor_grad_number):
		...
		grad.append(grad_px)
		var grad_text = " " + str(float(x_min) + n * float(x_axis_range)/(hor_grad_number-1))
		grad.append(grad_text)
